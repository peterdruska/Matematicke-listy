...
pygame.display.set_caption("Bazénový vysávač – náhodne")
    ...
    while True:
        ...
        if not simulation_done:
            ...
            while drawn_steps < target_steps:
                prev_x, prev_y = x, y
                x += dx * STEP_DISTANCE_PX
                y += dy * STEP_DISTANCE_PX

                if not RECT.collidepoint(x, y):
                    if x <= RECT.left:
                        angle = math.radians(180 + random.randint(90, 270))
                        x = RECT.left + 1
                    elif x >= RECT.right:
                        angle = math.radians(random.randint(90, 270))
                        x = RECT.right - 1
                    elif y <= RECT.top:
